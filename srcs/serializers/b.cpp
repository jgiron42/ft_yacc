...
BSerializer::BSerializer(Parser::Config &c, LALR &l) : config(c), lalr(l) {}

void BSerializer::generate() {
	this->build();

	this->generator.generate(SKELETONS_PATHS "/b.skl",
			commandLine.root + commandLine.file_prefix + ".tab.b");
}

void BSerializer::build() {
...
std::string BSerializer::serialize_sym_def() const {
	std::string ret;
...
std::string BSerializer::serialize_rules() {
	std::string ret;
...
std::string BSerializer::substitute_action(const LALR::Rule &rule) {
...
std::string BSerializer::serialize_transitions(size_t &size) const {
...
template<typename T>
std::string BSerializer::serialize_array(const std::vector<T> &array) const {
	std::string ret;
	bool is_first = true;
